...
def minimax(node,depth,maximizer,sourceDict,misses):
    # print(depth)
    print(len(node))
    plays=getPlays(node,sourceDict[len(node)],misses)
    if depth==0 or "_" not in node:
        if node not in plays:
            if maximizer:
                return 100000+depth, None, None
            else:
                return -100000-depth, None, None
        elif maximizer:
    ...
    if maximizer:
        ...
        for move in possibleMoves:
            child=getPlays(node,sourceDict,misses,guess=move)
            optimalSubChild=evalChild(child,node,maximizer)
            if move not in optimalSubChild:
                misses+=move
            if optimalSubChild!="":
                value,_,_=minimax(optimalSubChild,depth-1,False,sourceDict,misses)
        ...
        for move in possibleMoves:
            ...
            if move not in optimalSubChild:
                misses+=move
            if optimalSubChild!="":
                value,_,_=minimax(optimalSubChild,depth-1,True,sourceDict,misses)
